...
def _apply_fx(df, fx, base_col='txn_amt', ccy_col='currency_type', date_col='txn_date'):
    """
    【修正】正確對應 df 的幣別欄位與 fx['ccy']，方向固定為「1 外幣 = X TWD」，
    並在訓練/驗證/測試一致套用。
    """
    ...
    merged['rate_to_twd'] = merged['rate_to_twd'].fillna(1.0)  # 未知幣別先視為 1.0
...
def build_features(df_txn, df_alert, fx=None):
    ...
    for col in date_cols:
        if col in feat.columns:
            feat[col] = pd.to_datetime(col, errors='coerce')
...
def _time_split_for_cv(train_df, y_train, features, val_ratio=0.25):
    ...
    if y_val.sum() == 0:
        print("[WARN] 驗證集中沒有正樣本！F1-score 將為 0。")
...
def _try_params_and_find_best_f1(X_tr, y_tr, X_val, y_val,
    ...
    for pset in params_list:
        ...
        print(f"[INFO] 結果 - AUC: {auc_score:.4f}, 最佳 F1: {f1:.4f} @ 門檻={best_thresh:.2f}")
...
def run_pipeline():
    ...
    print(f"[INFO] 原始訓練集: {len(train_df)} 筆 (正樣本: {y_train.sum()})")
    ...
    print(f"[INFO] 使用 {len(features)} 個數值特徵訓練。")
    ...
    print(f"[CV-RESULT] 最佳 Thresh: {best_thresh:.4f}")
    ...
    y_hat_val_best = (proba_val_best >= best_thresh).astype(int)
